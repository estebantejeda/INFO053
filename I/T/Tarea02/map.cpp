...
int main(int argc, char **argv){
    ...
    for (int i = 0; i < N/1000; i++){
        for(int j = i*1000; j < i*1000+950; j++){
            M.insert(make_pair(to_string(1+rand()%(1000001-1)), 1+rand()%(1000001-1)));
        }
    ...
    for (int i = 0; i < N/1000; i++) {
        double t_1 = omp_get_wtime();
        M.find(to_string(numbers.at(1+rand()%(1000001-1))));
        double t_2 = omp_get_wtime();
    ...
    for (int i = 0; i < N/1000; i++){
        double t_1 = omp_get_wtime();
        for (int j = i*1000+950; j < i*1000+1000; j++){
            M.erase(to_string(numbers.at(1+rand()%(1000001-1))));
        }
        double t_2 = omp_get_wtime();
